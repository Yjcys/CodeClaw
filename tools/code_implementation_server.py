...
def _looks_like_summary_instead_of_code(file_path: str, content: str) -> bool:
    """Detect narrative summary text mistakenly written into source code files."""
    code_extensions = (".py", ".js", ".ts", ".tsx", ".jsx", ".java", ".go", ".rs")
    if not file_path.lower().endswith(code_extensions):
        return False
    if not isinstance(content, str):
        return False

    # Only inspect the header region where this failure pattern appears.
    ...
    matched = sum(1 for marker in markers if marker in header)
    return matched >= 2
...
async def write_file(
    ...
    try:
        ...
        if _looks_like_summary_instead_of_code(file_path, content):
            result = {
                "status": "error",
                "message": (
                    "Rejected write: content looks like a narrative implementation summary, "
                    "not executable source code. Please write actual code only."
                ),
                "file_path": file_path,
            }
            log_operation(
                "write_file_rejected_summary_content",
                {"file_path": file_path, "size_bytes": len(content.encode("utf-8"))},
            )
            return json.dumps(result, ensure_ascii=False, indent=2)
            ...
            try:
                if _looks_like_summary_instead_of_code(file_path, content):
                    results["files"][file_path] = {
                        "status": "error",
                        "message": (
                            "Rejected write: content looks like a narrative implementation "
                            "summary, not executable source code."
                        ),
                        "size_bytes": 0,
                        "lines_written": 0,
                        "backup_created": False,
                    ...
                    results["summary"]["failed"] += 1
                    log_operation(
                        "write_file_multi_rejected_summary_content",
                        {
                            "file_path": file_path,
                            "size_bytes": len(content.encode("utf-8")),
                            "batch_operation": True,
